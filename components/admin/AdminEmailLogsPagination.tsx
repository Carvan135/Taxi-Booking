...
type AdminEmailLogsPaginationProps = {
...
export function AdminEmailLogsPagination({
  query,
  page,
  totalPages,
  totalCount,
  pageSize,
}: AdminEmailLogsPaginationProps) {
  ...
  const prevHref =
    page > 1
      ? `/admin/emails${buildAdminEmailLogsQueryString({ ...query, page: page - 1 })}`
      : null;
  const nextHref =
    page < totalPages
      ? `/admin/emails${buildAdminEmailLogsQueryString({ ...query, page: page + 1 })}`
